...
func BasicAuthForRealm(accounts Accounts, realm string) webapp.HandlerFunc {
	...
	return func(ctx *webapp.Context) {
		...
		user, ok := searchCredential(pairs, ctx.Request.Header.Get("Authorization"))
		if !ok {
...
func BasicAuth(accounts Accounts) webapp.HandlerFunc {
